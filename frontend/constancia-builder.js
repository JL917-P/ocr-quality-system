...
function parseConstanciaDate(dateText) {
      ...
      function buildConstanciaHtml(constancia, catalog, clients = null) {
        ...
        // user01: tipografía/padding según cantidad de productos (pocos = más alto; muchos = compacto)
        const user01ItemCount = Math.max(items.length || 1, 1);
        const user01FontPx = Math.min(11, Math.max(6.2, 12.8 - user01ItemCount * 0.2));
        const user01PadPx = Math.min(6, Math.max(1.2, user01FontPx * 0.38));
        const user01FitStyle = user01Layout
          ? `--u01-n:${user01ItemCount};--u01-fs:${user01FontPx.toFixed(2)}px;--u01-pad:${user01PadPx.toFixed(2)}px;--u01-head:12mm;`
          : "";
        ...
        const user01WideCss = user01Layout
          ? `
                /* user01: márgenes laterales + tabla que aprovecha alto disponible */
                ...
                body.user01-wide .u01-table-slot .data th,
                body.user01-wide .u01-table-slot .data td {
                  font-size: var(--u01-fs);
                ...
                body.user01-wide .u01-table-slot .data tbody tr {
                  height: calc((100% - var(--u01-head)) / var(--u01-n));
                }
                body.user01-wide .u01-table-slot .data.quality thead {
                  height: var(--u01-head-q, 16mm);
                }
                body.user01-wide .u01-table-slot .data.quality tbody tr {
                  height: calc((100% - var(--u01-head-q, 16mm)) / var(--u01-n));
                }
